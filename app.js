...
function copiarLinkFamiliar() {
  if(window.currentSyncUrl) {
    navigator.clipboard.writeText(window.currentSyncUrl).then(() => {
      toast('�Enlace copiado! Env�alo a tu pareja.');
    });
  }
}
...
function importarDatosFamiliar() {
  ...
  // Merge simple: combinar transacciones y deudas �nicas
  ...
  syncDataPendiente.deudas.forEach(d => {
    ...
    if(!existente) {
      ...
      // Conservar el saldo m�s reciente (suponiendo que haya actualizaci�n, o simplemente mantener actual)
  ...
  // Merge de configuraci�n b�sica si es nuevo
  ...
  toast('? Datos familiares fusionados con �xito');
