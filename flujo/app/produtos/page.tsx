...
export default function ProductosPage() {
  ...
  return (
    <>
      <PageHeader title="Productos" breadcrumb="Productos" description="Inventario, precios y proveedores."
        actions={<Button size="sm"><Plus className="h-4 w-4" /> Nuevo producto</Button>}
      />
      <div className="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
        <StatCard label="Productos" value={String(produtos.length)} icon={Package} hint="activos" />
        ...
        <StatCard label="Stock bajo" value={String(produtos.filter((p) => p.stock <= p.stockMinimo).length)} icon={AlertTriangle} hint="alertas" />
      </div>
      <DataTable columns={columns} data={produtos} searchKey="nombre" searchPlaceholder="Buscar productos..." />
    </>
  );
}
